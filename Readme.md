...
* 網掛けのボックス（定義：macroMP.sty）
```
\begin{shadelinebox}
\end{shadelinebox}
```

* ラウンド罫のボックス（定義：macroMP.sty）
```
\begin{screenbox}
\end{screenbox}
```

* 引用（定義：macroMP.sty（書体が特殊なので代替フォントをあてています．））
```
\begin{Quote}
\end{Quote}
```
...
| | |
| multicol.sty   | texlive2015で収録されているものはpLaTeXのtombowオプションと相性が悪いため正常に動くものを念のために添付 | 
